#include "itcstocksco1.hpp"

#include "config.h"


// constructor
itcstocksco1::itcstocksco1(name self, name code, datastream<const char *> ds) : contract(self, code, ds),
...
void itcstocksco1::init(name recipient, time_point_sec start, time_point_sec finish)
{
    ...
    asset goal = asset(GOAL, symbol("ITC", 8));
}

// issuance of only reserved QUI
void itcstocksco1::issue(name to, asset quantity, uint64_t _class, std::string memo)
{
    require_auth(this->state.recipient);
    check( is_account( to ), "to account does not exist" );
    check( quantity.symbol == sy_itc ,"Can issue only ITC coins");
    switch(_class){
        case 1:
    ...
    this->inline_issue(to, quantity, "Issue ITC tokens to reserved class successfull");

}

// update contract balances and send QUI tokens to the investor
void itcstocksco1::handle_investment(name investor, uint64_t tokens_to_give)
{
    ...
    asset entire_tokens = asset(tokens_to_give, symbol("ITC", 8));
...
void itcstocksco1::buyitc(name from, name to, asset quantity, std::string memo)
{
    print(from);
    print(to);
    print(quantity);
    ...
    asset amount = asset(tokens_to_give, symbol("ITC", 8));

    this->inline_transfer(this->_self, from, amount, "Crowdsale deposit successful, bought ITC tokens"); 
    }
}

// used for trading.
void itcstocksco1::transfer(eosio::name from, eosio::name to, eosio::asset quantity, std::string memo)
{
    require_auth(from);
...
void itcstocksco1::rate()
{
    print("Rate of 1 ITC token:");
    print(RATE);
}

void itcstocksco1::checkgoal()
{
...
void itcstocksco1::withdraw()
{
    ...
    //  itcstocksco1 deposit of eos deleted from table (which equals to total eoses in state)
    this->state.total_eoses.amount = 0;
    auto it = this->deposits.find(("itctokens321"_n).value);
...
extern "C" void apply(uint64_t receiver, uint64_t code, uint64_t action)
{
    ...
    {
        eosio::execute_action(eosio::name(receiver), eosio::name(code), &itcstocksco1::transfer);
    }
    else if (code == receiver) // for other direct actions
    {
        switch (action)
        {
            EOSIO_DISPATCH_HELPER(itcstocksco1, (init)(transfer)(issue)(pause)(rate)(checkgoal)(withdraw));
        }
    }
}
